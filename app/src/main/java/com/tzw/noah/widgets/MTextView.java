...
//    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //只有一行时
        if (oneLineWidth != -1) {
            height = 0;// getMeasuredHeight() / 2 - contentList.get(0).height / 2;
        }

        for (int i = 0; i < contentList.size() && i < getMaxLines(); i++) {
            //绘制一行
            ...
            for (int j = 0; j < line.line.size(); j++) {
                ...
                if (ob instanceof ImageSpan) {
                    ...
                    if (ob instanceof StringWithColor) {
                        ...
                        RectF p = new RectF(realDrawedWidth, height, realDrawedWidth + width, height + line.height);
                        clickArea.add(p);
                    }

                    canvas.drawText(textStr, realDrawedWidth, height + line.height - paint.getFontMetrics().bottom, paint);
                    realDrawedWidth += width;
                }
            }
...
//        com.tzw.noah.logger.Log.log("bbb", "measureContentHeight, "+text.toString());
...
//        com.tzw.noah.logger.Log.log("bbb", "getCachedData, " + text.toString());
...
//        com.tzw.noah.logger.Log.log("bbb", textStr.toString());
        text = textStr;
        mText = textStr;
        obList.clear();
        //  contentList.clear();
        ...
        if (textStr instanceof SpannableString) {
            ...
            ClickableSpan[] clickspans = ss.getSpans(0, ss.length(), ClickableSpan.class);
            if (clickspans.length > 0) {
            }
        }

        //对isList重新排序,按start最小的开始排序
        if (isList.size() > 1) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP && mText != null) {
            ClickableSpan[] links = ((Spannable) mText).getSpans(0,
                    mText.length(), ClickableSpan.class);
            if (links.length > 0) {
                if (isInClickArea(event.getX(), event.getY()))
                    links[0].onClick(this);
                else links[0].onClick(null);
                    return handled = true;
            }
        }
...
//        float baseHeight = 0, baseWidth = 0;
        for (int i = 0; i < clickArea.size(); i++) {
            RectF p = clickArea.get(i);
