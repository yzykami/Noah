...
public class MyCommentActivity extends MySwipeBackActivity implements OnRecyclerLoadMoreListener, MediaListListener {
...
//    View maskView;
    Context mContext = MyCommentActivity.this;

    InputFragment frame_input;

    private AssemblyRecyclerAdapter adapter;
    MyCommentActivity instance;
...
//            loadData();
    ...
    private void doSearch() {
        ...
        NetHelper.getInstance().mediaMixCommentsList(typeId, articleId, DataCenter.pagesize, new StringDialogCallback(mContext) {
            @Override
            ...
            public void onResponse(IMsg iMsg) {
                try {
                    if (iMsg.isSucceed()) {
                        List<MyComment> list = MyComment.loadList(iMsg);
//                        items =
                        if (list == null)
                            list = new ArrayList();

                        items = new ArrayList<>();
                        for (MyComment fav : list) {
                        ...
                        if (list.size() >= 1) {
                            articleId = list.get(list.size() - 1).memberCommentId;
                        }
...
//            return;
//        NetHelper.getInstance().mediaMixComment(0, ids, 0, new StringDialogCallback(mContext) {
//            @Override
//            public void onFailure(Call call, IOException e) {
//                isloading = false;
