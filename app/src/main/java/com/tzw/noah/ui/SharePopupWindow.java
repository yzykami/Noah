...
public class SharePopupWindow extends PopupWindow {
    ...
    public static SharePopupWindow create(Activity activity, OnItemClickListener onItemClickListener) {
        return new SharePopupWindow(activity, onItemClickListener);
    }

    private SharePopupWindow(Activity activity, OnItemClickListener onItemClickListener) {
        super(activity);
        mContext = activity;
        mActivity = activity;
        items = new ArrayList<>();
...
//    public SharePopupWindow addItem(String title) {
//        return addItem(title, 0);
//    }

    public SharePopupWindow addItem(String title, int resId) {
        Item item = new Item();
        item.title = title;
        item.resId = resId;
        items.add(item);
    ...
    public void show() {
        ...
        for (int i = 0; i < items.size(); i++) {
            View itemView = inflater.inflate(R.layout.share_popupwindow_item, null);
            ...
            final int finalI = i;
            RelativeLayout rl = (RelativeLayout) itemView.findViewById(R.id.rl_item);
